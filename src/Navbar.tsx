...
function Navbar({ activeId }: { activeId: string }) {
  ...
  return (
    <div className="relative w-full text-[#f8fafc] h-14 items-center flex justify-between px-6 bg-[#050505]/90 backdrop-blur-sm border-b border-[#1d4ed8]/20 z-50">
      ...
      <ul className="hidden md:flex gap-4 lg:gap-7 text-sm lg:text-base font-medium h-20 items-center">
        {links.map((link) => (
          <li key={link.id}>
            <a
      ...
      <div className="hidden md:flex items-center gap-3 lg:gap-4">
        {langButton}
        <a href={socialLinks.github} target="_blank" rel="noopener noreferrer" aria-label="GitHub" className="hidden lg:inline hover:text-[#2563eb] transition-colors">
          <Github size={22} />
        </a>
        <a href={socialLinks.linkedin} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" className="hidden lg:inline hover:text-[#2563eb] transition-colors">
          <Linkedin size={22} />
        </a>
        <a href={`mailto:${socialLinks.email}`} aria-label="Email" className="hidden lg:inline hover:text-[#2563eb] transition-colors">
          <Mail size={22} />
        </a>
      </div>
      ...
      {open && (
        <div className="absolute top-14 left-0 w-48 h-screen bg-[#050505]/95 backdrop-blur-sm border-r border-[#1d4ed8]/20 flex flex-col gap-6 p-6 md:hidden shadow-lg">
          {links.map((link) => (
            <a
              key={link.id}
              className={linkClass(link.id)}
